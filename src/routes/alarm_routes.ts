...
router.get('/:alarmId',(req,res)=>{
    const controller = new AlarmController();
    controller.getAlarm(req.params.alarmId)
...
router.delete('/:alarmId',(req,res)=>{
    const controller =new AlarmController();
    controller.deleteAlarm(req.params.alarmId).then((_)=>{res.send("deleted");}).catch((err)=>{
        console.log(err);
        res.status(404).send('the requested alarm not found ');
    });
});
...
router.post('/create',(req,res)=>{
    const controller =new AlarmController();
    controller.createAlarm(req.body).then((_)=>{res.send("Created");
}).catch((err)=>{
    console.log('err')
...
router.put("/update/:alarmId", (req, res) => {
    const controller = new AlarmController();
    controller.updatealarm(req.params.alarmId, req.body)
      .then((response) => {
