...
router.get('/:announcementId',(req,res)=>{
    const controller = new AnnouncementController();
    controller.getAnnouncement(req.params.announcementId)
...
router.delete('/:announcementId',(req,res)=>{
    const controller =new AnnouncementController();
    controller.deleteAnnouncement(req.params.announcementId).then((_)=>{res.send("deleted");}).catch((err)=>{
        console.log(err);
        res.status(404).send('the requested announcement not found ');
    });
});
...
router.post('/create',(req,res)=>{
    const controller =new AnnouncementController();
    controller.createannouncement(req.body).then((_)=>{res.send("Created");
}).catch((err)=>{
    console.log('err')
...
router.put("/update/:announcementId", (req, res) => {
    const controller = new AnnouncementController();
    controller.updateAnnouncement(req.params.announcementId, req.body)
      .then((response) => {
