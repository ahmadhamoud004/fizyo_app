...
router.get('/:enumId',(req,res)=>{
    const controller = new EnumController();
    controller.getEnum(req.params.enumId)
...
router.delete('/:enumId',(req,res)=>{
    const controller =new EnumController();
    controller.deleteEnum(req.params.enumId).then((_)=>{res.send("deleted");}).catch((err)=>{
        console.log(err);
        res.status(404).send('the requested enum not found ');
    });
});
...
router.post('/create',(req,res)=>{
    const controller =new EnumController();
    controller.createEnum(req.body).then((_)=>{res.send("Created");
}).catch((err)=>{
    console.log('err')
...
router.put("/update/:enumId", (req, res) => {
    const controller = new EnumController();
    controller.updateEnum(req.params.enumId, req.body)
      .then((response) => {
