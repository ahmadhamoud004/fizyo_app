...
router.get('/:notificationId',(req,res)=>{
    const controller = new NotificationController();
    controller.getNotification(req.params.notificationId)
...
router.delete('/:notificationId',(req,res)=>{
    const controller =new NotificationController();
    controller.deleteNotification(req.params.notificationId).then((_)=>{res.send("deleted");}).catch((err)=>{
        console.log(err);
        res.status(404).send('the requested notification not found ');
    });
});
...
router.post('/create',(req,res)=>{
    const controller =new NotificationController();
    controller.createNotification(req.body).then((_)=>{res.send("Created");
}).catch((err)=>{
    console.log('err')
...
router.put("/update/:notificationId", (req, res) => {
    const controller = new NotificationController();
    controller.updateNotification(req.params.notificationId, req.body)
      .then((response) => {
